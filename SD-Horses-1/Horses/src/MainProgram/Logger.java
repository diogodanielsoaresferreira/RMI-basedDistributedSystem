...
public class Logger implements LoggerInterface{
     ...
     */
    @Override
    public synchronized void updateTrack(int trackDistance, int raceNumber){
     ...
     */
    public synchronized void cleanRaceVariables(){
        /**
     ...
     */
    @Override
    public synchronized void updateBrokerState(BrokerState state){
     ...
     */
    @Override
    public synchronized void updateSpectatorState(int id, SpectatorState state){
       for(int i=0; i<spectatorIds.length; i++)
     ...
     */
    @Override
    public synchronized void updateHorseState(int id, HorseJockeyState state){
        for(int i=0; i<horseIds.length; i++)
            if(horseIds[i]==id)
                horseState[i] = state;
        this.printLog();
     ...
     */
    @Override
    public synchronized void updateHorseInRace(int id, int iteration, int distance){
        for(int i=0; i<horseIds.length; i++)
            if(horseIds[i]==id){
     ...
     */
    @Override
    public synchronized void updateSpectatorBet(int id, int betSel, int betAmmount){
        for(int i=0; i<spectatorIds.length; i++)
            if(spectatorIds[i]==id){
     ...
     */
    @Override
    public synchronized void updateSpectatorMoney(int id, int money){
        for(int i=0; i<spectatorIds.length; i++)
            if(spectatorIds[i]==id)
     ...
     */
    @Override
    public synchronized void reportResults(int[] horseIds, int[] horsePositions){
